...
export default function HeroSection() {
  ...
  return (
    <section className="hero-section hero-section--cosmos" ref={ref}>
      {/* 3D ネットワーク背景 */}
      <div className="hero-cosmos-bg">
        <HeroCanvas />
        <div className="hero-cosmos-vignette" />
      </div>

      {/* テキスト（前面レイヤー） */}
      <div className="hero-container hero-container--overlay">
        <div className="hero-text hero-text--center">
          <h1 className={`hero-title hero-title--cosmos reveal stagger-1 ${isVisible ? 'visible' : ''}`}>
            現場のあなたと、<br />
            共につくる。
          </h1>
          <p className={`hero-description hero-description--cosmos reveal stagger-2 ${isVisible ? 'visible' : ''}`}>
